...
let renderHandler = function (req, res) {
    res.render('home/info/announcement', {
        layout: 'home'
    });
};
...
let createHandler = async function (req, res) {
            ...
            if (req.files['announcementImg'] != undefined) {
                let params = {
                    ...
                    Delete: { // required
                        Objects: [ // required
                            {
                                Key: req.files['announcementImg'][0].key // required
                            },
                        ],
                    },
                };
    ...
    try {
        ...
        if (req.files['announcementImg'] != undefined) {
            ...
            sql = `INSERT INTO Announcement (adminId, title, imageSrc, imageKey) VALUES (?, ?, ?, ?);`;
            values = [adminId, title, req.files['announcementImg'][0].location, req.files['announcementImg'][0].key];
    ...
    // Log 
    req.logger.verbose(`account[${req.user.account}] role[${req.user.role}] create a new announcement title[${title}]`);
...
let deleteHandler = async function (req, res) {
    ...
    // Log 
    for (let i = 0; i < deleteData.length; i++) {
        req.logger.verbose(`account[${req.user.account}] role[${req.user.role}] delete annonuce id[${deleteData[i].id}]`);
    }
