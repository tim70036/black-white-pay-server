...
let renderHandler = async function(req,res){
    ...
    try {
        ...
        if(results.length <= 0) throw Error(`cannot find ad id[${id}] of user role[${req.user.role}] roleId[${req.user.roleId}]`);

        return res.render('home/detail/ad', {layout : 'home', ad: results[0]});
...
let editHandler = async function(req,res){
    ...
    try {
        ...
        if (req.files['adImg'] != undefined) {
            ...
            sql = `UPDATE Ad
                            ...
                            WHERE id=?
                            `;
            values = [title, req.files['adImg'][0].location, req.files['adImg'][0].key, id];
            sqlString = mysql.format(sql, values);
    ...
    // Log
    req.logger.verbose(`account[${req.user.account}] role[${req.user.role}] update ad id[${id}]`);
